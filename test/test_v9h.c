...
int main(void) {
    ...
    struct { int m, n, k, lda, ldb, ldc, ta, tb; } cases[] = {
        ...
        {130, 258, 70,  74,  260, 260, 0, 0},   // NN 边界 (直通 B 越界回归: hipblas m 列 > k 时读界)
        {256, 512, 256, 256, 512, 512, 0, 1},   // NT 大 N' (hipblas transA=T, N'=512>K=256:
                                                //   op_b==T 直跑读域=物理 (m-1)*lda+k, 恰好贴界验证)
