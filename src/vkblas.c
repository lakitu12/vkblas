...
VkPipeline mpipe[8];  // matvec: [0]=TB0 [1]=TB1 (M==1 单 pass), [2]=TB0 sk [3]=TB1 sk
    ...
    VkPipeline mpipe_m[2];  // M 行 matvec split-k (op_b==T 缓存 Bt 用): [0]=fp16, [1]=bf16
    ...
    struct tc_entry { const void* ptr; uint32_t N, K, ldb; int elem; const void* base;
...
// B 转置缓存 (定义见后): elem=4/2, dtype=0/1 (2B 时)
static int tc_get(const void* ptr, uint32_t N, uint32_t K, uint32_t ldb, int dir, int elem,
                  VkBuffer* out, size_t* out_size, int* built, int dtype);
...
static void init_vkblas(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        VkComputePipelineCreateInfo cp8 = { .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
            .stage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0,
                       VK_SHADER_STAGE_COMPUTE_BIT, sm, "main", NULL },
            .layout = g.pl };
    ...
    for (int i = 0; i < 4; i++) {
        ...
        VkComputePipelineCreateInfo cp9 = { .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
            .stage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0,
                       VK_SHADER_STAGE_COMPUTE_BIT, sm, "main", NULL },
            .layout = g.pl };
    ...
    for (int d = 0; d < 2; d++) {
        for (int i = 0; i < 4; i++) {
            ...
            if (load_spv(h128x64names[d][i], &sm) == 0) {
                VkComputePipelineCreateInfo hp = { .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
                    .stage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0,
                               VK_SHADER_STAGE_COMPUTE_BIT, sm, "main", NULL },
                    .layout = g.pl };
    ...
    for (int d = 0; d < 2; d++) {
        ...
        if (load_spv(mmnames[d], &sm) == 0) {
            VkComputePipelineCreateInfo mcp = { .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
                .stage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0,
                           VK_SHADER_STAGE_COMPUTE_BIT, sm, "main", NULL },
                .layout = g.pl };
            vk_check(vkCreateComputePipelines(g.dev, VK_NULL_HANDLE, 1, &mcp, NULL, &g.mpipe_m[d]), "matvec m pipe");
            vkDestroyShaderModule(g.dev, sm, NULL);
        }
        if (load_spv(rmnames[d], &sm) == 0) {
            VkComputePipelineCreateInfo rhp = { .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
                .stage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0,
                           VK_SHADER_STAGE_COMPUTE_BIT, sm, "main", NULL },
                .layout = g.tpl };
            vk_check(vkCreateComputePipelines(g.dev, VK_NULL_HANDLE, 1, &rhp, NULL, &g.rhpipe_m[d]), "rh m pipe");
            vkDestroyShaderModule(g.dev, sm, NULL);
        }
    }
...
//   实验性 v8 (128×64, 32 acc) / v9 (128×128 无冲突读) 与 v7 持平 (gfx803 LDS 中转 ~50% 效率墙)
// NN 实测定界 (gfx803, 2026-08-24, VKBLAS_TILE128=0 强制真 v6 对照):
//     方阵/近方阵: v7 全面胜 (4096³: v6 73ms / v7 52ms; M<256 走 v6 保持并行度)
//     M>N 窄 N: v7 在 (4096,512) 大胜 (1.6x); 中 shape (1024-2048) v6 微胜 ≤8% (维持 v7 简化)
...
static vkblas_status_t gemm_h_direct(int dtype, int hip_physical, vkblas_op_t op_a, vkblas_op_t op_b,
    ...
    for (uint32_t i = 0; i < batch; i++) {
        ...
        if (op_b == VKBLAS_OP_T && M >= 2 && M <= 64 &&
            (lda & 1u) == 0 && (ldc & 1u) == 0 &&
            getenv("VKBLAS_CACHE_TRANSPOSE") != NULL &&
            g.mpipe_m[dtype] != VK_NULL_HANDLE) {
            VkBuffer bBt; size_t bt_sz; int tbuilt;
            if (tc_get((const void*)pb, N, K, ldb, 0, 2, &bBt, &bt_sz, &tbuilt, dtype) == 0) {
                uint32_t ld_t = N + (N & 1u);
                size_t ba_m = ((size_t)(M - 1) * lda + K) * 2;
                size_t bb_m = ((size_t)(K - 1) * ld_t + N) * 2;
                size_t bc_m = ((size_t)(M - 1) * ldc + N) * 2;
                rc = run_matvec_sk_m_vk(dtype, pa, ba_m, lda, bBt, bb_m,
                                        pc, bc_m, ldc, K, N, ld_t, M, alpha, beta);
                if (rc == 0) {
                    release_ptr(mA, bA); release_ptr(mB, bB); release_ptr(mC, bC);
                    if (trace)
                        fprintf(stderr, "[vk] %s direct: batch %u done via M-matvec\n", dtype ? "bf16" : "f16", i);
                    pa += stride_a * 2; pb += stride_b * 2; pc += stride_c * 2;
                    continue;
                }
                rc = 0;  // matvec 失败 → 落回 GEMM 分支
            }
        }
        if (op_b == VKBLAS_OP_N) {
            ...
            if (op_b == VKBLAS_OP_T && getenv("VKBLAS_CACHE_TRANSPOSE") != NULL) {
                ...
                if (tc_get((const void*)pb, N, K, ldb, 0, 2, &bBt, &bt_sz, &tbuilt, dtype) == 0) {
                    uint32_t ld_t = N + (N & 1u);
                    okt = run_gemm_h128(dtype, variant ^ 1, bA, ba_e * 2, bBt, bt_sz, bC, bc_e * 2,
                                        M, N, K, lda, ld_t, ldc, 1, 0, 0, 0, alpha, beta, 1) == 0;
                }
            }
            if (!okt) {
                rc = run_gemm_h128(dtype, variant, bA, ba_e * 2, bB, bb_e * 2, bC, bc_e * 2,
            ...
            if (op_b == VKBLAS_OP_T && getenv("VKBLAS_CACHE_TRANSPOSE") != NULL) {
                ...
                if (tc_get((const void*)pb, N, K, ldb, 0, 2, &bBt, &bt_sz, &tbuilt, dtype) == 0) {
                    uint32_t ld_t = N + (N & 1u);
                    okt = run_gemm_h(dtype, variant ^ 1, bA, ba_e * 2, bBt, bt_sz, bC, bc_e * 2,
                                     M, N, K, lda, ld_t, ldc, 1, 0, 0, 0, alpha, beta, 1) == 0;
                }
            }
            if (!okt) {
...
static int run_gemm_vk128v9(int variant, VkBuffer bA, size_t ba, VkBuffer bB, size_t bb,
                            ...
                            uint32_t batch, int64_t stride_a, int64_t stride_b, int64_t stride_c,
                            float alpha, float beta, int submit) {
    if (g.pipe128v9[variant] == VK_NULL_HANDLE) return -1;
    VkDescriptorBufferInfo db[3] = {
        {bA, 0, ba}, {bB, 0, bb}, {bC, 0, bc} };
    VkWriteDescriptorSet wds[3];
    for (int i = 0; i < 3; i++) {
    ...
    if (submit) cmd_begin();
    vkCmdBindPipeline(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.pipe128v9[variant]);
    vkCmdBindDescriptorSets(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.pl, 0, 1, &g.dset, 0, NULL);
    vkCmdPushConstants(g.cmd, g.pl, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc), &pc);
    vkCmdDispatch(g.cmd, pc.Mt * batch, pc.Nt, 1);
...
// M 行 matvec split-k (decode m=2..64): C[M][N] = A[M][K] · Bt[K][N]
// 通用 GEMM 64×64 tile 在 M 小时只有 Nt 个 wg (并行度不足→0.9ms);
// 这里每 wg 处理 1 行×512 列×K 段, dispatch (Nt, M, seg) 几百 wg.
// pass1: matvec_sk_m → Sk[seg][M][N] fp32; pass2: split_k_reduce_m → C 2B
// 前提 (host 保证): A/Bt/C 行步长 lda/ldb/ldc 全偶 (2B uint 对齐), Bt 紧密缓存
static int run_matvec_sk_m_vk(int dtype, const void* A, size_t ba, uint32_t lda,
                              VkBuffer bB, size_t bb, void* C, size_t bc, uint32_t ldc,
                              uint32_t K, uint32_t N, uint32_t ldb, uint32_t M,
                              float alpha, float beta) {
    if (g.mpipe_m[dtype] == VK_NULL_HANDLE || g.rhpipe_m[dtype] == VK_NULL_HANDLE) return -1;
    VkBuffer bA, bC;
    VkDeviceMemory mA, mC;
    VkDeviceSize oA, oC;
    if (import_ptr(A, ba, &bA, &mA, &oA) != 0) return -1;
    ...
    // 每 wg 覆盖 R 行 (B 复用 M/R 次): dispatch x=N/512 (每线程 2 列), y=M/R, z=seg
    const char* mrenv = getenv("VKBLAS_M_ROWS");
    uint32_t R = mrenv && *mrenv ? (uint32_t)strtoul(mrenv, NULL, 10) : 16;
    if (R < 1) R = 1;
    if (R > 16) R = 16;
    if (R > M) R = M;
    ...
    if (nsk && *nsk) { long v = strtol(nsk, NULL, 10); if (v >= 1 && v <= 64) seg = (uint32_t)v; }
    size_t need = (size_t)seg * M * N * 4;
    if (g.sk_buf == VK_NULL_HANDLE || need > g.sk_buf_size) {
        if (g.sk_buf != VK_NULL_HANDLE) {
            vkFreeMemory(g.dev, g.sk_mem, NULL);
            vkDestroyBuffer(g.dev, g.sk_buf, NULL);
    ...
    uint32_t k_split = (K + seg - 1) / seg;
    if (getenv("VKBLAS_TRACE"))
        fprintf(stderr, "[vk] mv_m: M=%u K=%u N=%u lda=%u ldb=%u R=%u Nt=%u Mg=%u seg=%u k_split=%u ba=%zu bb=%zu bc=%zu\n",
                M, K, N, lda, ldb, R, Nt, Mg, seg, k_split, ba, bb, bc);
    ...
    cmd_begin();
    {
        ...
        vkUpdateDescriptorSets(g.dev, 3, wds, 0, NULL);
        struct { uint32_t K, N, lda, ldb, M, k_split, R, pad; } pc = { K, N, lda, ldb, M, k_split, R, 0 };
        vkCmdBindPipeline(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.mpipe_m[dtype]);
        vkCmdBindDescriptorSets(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.pl, 0, 1, &g.dset, 0, NULL);
        vkCmdPushConstants(g.cmd, g.pl, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc), &pc);
        vkCmdDispatch(g.cmd, Nt, Mg, seg);
        VkBufferMemoryBarrier bmb = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
            .srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT,
            .dstAccessMask = VK_ACCESS_SHADER_READ_BIT,
    ...
    }
    {
        ...
        vkUpdateDescriptorSets(g.dev, 2, wds, 0, NULL);
        struct { uint32_t N_, k_num, M_, ldc; float alpha, beta; } pc2 = {
            N, seg, M, ldc, alpha, beta };
        vkCmdBindPipeline(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.rhpipe_m[dtype]);
        vkCmdBindDescriptorSets(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE, g.tpl, 0, 1, &g.tdset, 0, NULL);
        vkCmdPushConstants(g.cmd, g.tpl, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc2), &pc2);
        vkCmdDispatch(g.cmd, (N + 511) / 512, M, 1);
    }
    cmd_end_submit("matvec m split-k submit");
    release_ptr(mA, bA); release_ptr(mC, bC);
    return 0;
}
...
// elem: 4 (fp32) 或 2 (fp16/bf16); dtype: 0=fp16 1=bf16 (elem==2 时用)
static int tc_get(const void* ptr, uint32_t N, uint32_t K, uint32_t ldb, int dir, int elem,
                  VkBuffer* out, size_t* out_size, int* built, int dtype) {
    *built = 0;
    for (int i = 0; i < g.tc_cnt; i++) {
        if (g.tc_tab[i].ptr == ptr && g.tc_tab[i].N == N &&
            g.tc_tab[i].K == K && g.tc_tab[i].ldb == ldb && g.tc_tab[i].elem == elem) {
            VkBuffer b = dir == 0 ? g.tc_tab[i].b_kn : g.tc_tab[i].b_nk;
            if (b != VK_NULL_HANDLE) { *out = b; *out_size = (size_t)N * K * elem; return 0; }
    ...
    VkBuffer bIn; VkDeviceMemory mIn; VkDeviceSize oIn;
    if (import_ptr(ptr, bb * elem, &bIn, &mIn, &oIn) != 0) return -1;
    const void* cbase = ic_block_base(ptr);
    const size_t need = (size_t)N * K * elem;
    ...
    }
    {
        ...
        if (elem == 4) {
            ...
            cmd_begin();
            if (dir == 0)
                transpose_h_into(bIn, bb * elem, bT0, need, N, K, ldb, &ldout0, dtype, 1, 0, 0, 0);
            else
                transpose_h_into(bIn, bb * elem, bT0, need, K, N, ldb, &ldout0, dtype, 1, 0, 0, 0);
            cmd_end_submit("tc build t0 submit");
            ldout0 = 0;
            cmd_begin();
            transpose_h_into(bT0, need, bT1, need, dir == 0 ? K : N, dir == 0 ? N : K,
                             dir == 0 ? N : K, &ldout0, dtype, 1, 0, 0, 0);
            cmd_end_submit("tc build t1 submit");
        }
    }
    release_ptr(mIn, bIn);
    {
        ...
        e->ptr = ptr; e->N = N; e->K = K; e->ldb = ldb; e->elem = elem; e->base = cbase;
...
// matvec split-k half, B 直接 VkBuffer (转置缓存): 同 run_matvec_sk_h
static int run_matvec_sk_h_vk(int dtype, const void* A, size_t ba,
                              VkBuffer bB, VkBuffer bC, size_t bc,
                              uint32_t K, uint32_t N, uint32_t ldb, uint32_t ldc,
                              float alpha, float beta) {
    VkPipeline p1 = g.mpipe[4 + dtype];
    VkPipeline p2 = g.rhpipe[dtype];
    if (p1 == VK_NULL_HANDLE || p2 == VK_NULL_HANDLE) return -1;
    VkBuffer bA; VkDeviceMemory mA; VkDeviceSize oA;
    if (import_ptr(A, ba, &bA, &mA, &oA) != 0) return -1;
    (void)oA;
    uint32_t Nt = (N + 511) / 512;
    uint32_t seg_occ = 144 / Nt + 1;          // 目标 ≥144 wg
    ...
    cmd_begin();
    {
        VkDescriptorBufferInfo db[3] = { {bA, 0, ba}, {bB, 0, (size_t)N * K * 2}, {g.sk_buf, 0, need} };
        VkWriteDescriptorSet wds[3];
        for (int i = 0; i < 3; i++) {
            wds[i] = (VkWriteDescriptorSet){ .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                .dstSet = g.dset, .dstBinding = (uint32_t)i, .descriptorCount = 1,
    ...
    cmd_end_submit("matvec half cached submit");
    release_ptr(mA, bA);
    return 0;
}
...
vkblas_status_t vkblas_gemm_f32(
    ...
    if (M == 1 && op_a == VKBLAS_OP_N && batch == 1 &&
        ...
        if (op_b == VKBLAS_OP_T && getenv("VKBLAS_CACHE_TRANSPOSE") != NULL &&
            ...
            if (tc_get(B, N, K, ldb, 0, 4, &bBt, &bt_sz, &built, 0) == 0)
...
static vkblas_status_t gemm_bf16_core(int hip_physical,
    ...
    if (M == 1 && op_a == VKBLAS_OP_N && batch == 1 &&
        (ldc & 1u) == 0 && (ldb & 1u) == 0 && (N & 1u) == 0 && g.mpipe[5] != VK_NULL_HANDLE) {
        size_t ba_h = (size_t)K * 2;
        size_t bc_h = (size_t)N * 2;
        int hrc = -1;
        if (op_b == VKBLAS_OP_N) {
            size_t bb_h = ((size_t)(K - 1) * ldb + N) * 2;
            hrc = run_matvec_sk_h(1, A, ba_h, B, bb_h, C, bc_h, K, N, ldb, ldc, alpha, beta);
        } else if (getenv("VKBLAS_CACHE_TRANSPOSE") != NULL) {
            // op_b==T: 权重转置缓存 → TB=0 matvec
            VkBuffer bBt; size_t bt_sz; int tbuilt;
            if (tc_get(B, N, K, ldb, 0, 2, &bBt, &bt_sz, &tbuilt, 1) == 0) {
                VkBuffer bCv; VkDeviceMemory mCv; VkDeviceSize oCv;
                if (import_ptr(C, bc_h, &bCv, &mCv, &oCv) == 0) {
                    (void)oCv;
                    hrc = run_matvec_sk_h_vk(1, A, ba_h, bBt, bCv, bc_h,
                                             K, N, N + (N & 1u), ldc, alpha, beta);
                    release_ptr(mCv, bCv);
                }
            }
        }
...
vkblas_status_t vkblas_gemm_f16(
    ...
    if (M == 1 && op_a == VKBLAS_OP_N && batch == 1 &&
        (ldc & 1u) == 0 && (ldb & 1u) == 0 && (N & 1u) == 0 && g.mpipe[4] != VK_NULL_HANDLE) {
        size_t ba_h = (size_t)K * 2;
        size_t bc_h = (size_t)N * 2;
        int hrc = -1;
        if (op_b == VKBLAS_OP_N) {
            size_t bb_h = ((size_t)(K - 1) * ldb + N) * 2;
            hrc = run_matvec_sk_h(0, A, ba_h, B, bb_h, C, bc_h, K, N, ldb, ldc, alpha, beta);
        } else if (getenv("VKBLAS_CACHE_TRANSPOSE") != NULL) {
            VkBuffer bBt; size_t bt_sz; int tbuilt;
            if (tc_get(B, N, K, ldb, 0, 2, &bBt, &bt_sz, &tbuilt, 0) == 0) {
                VkBuffer bCv; VkDeviceMemory mCv; VkDeviceSize oCv;
                if (import_ptr(C, bc_h, &bCv, &mCv, &oCv) == 0) {
                    (void)oCv;
                    hrc = run_matvec_sk_h_vk(0, A, ba_h, bBt, bCv, bc_h,
                                             K, N, N + (N & 1u), ldc, alpha, beta);
                    release_ptr(mCv, bCv);
                }
            }
